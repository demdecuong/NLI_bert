...
from utils import MODEL_CLASSES, compute_metrics, get_intent_labels, get_slot_labels, compute_metrics_intent
...
class Trainer(object):
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.logging_steps > 0 and global_step % self.args.logging_steps == 0:
                        ...
                        writer.add_scalar("Mean Intent Slot",
                                          results["mean_intent_slot"], _)
                ...
                if 0 < self.args.max_steps < global_step:
                    ...
                    break
            print("Train loss :", tr_loss/global_step)
            print("Train KL loss :", tr_kl_loss/global_step)
            if 0 < self.args.max_steps < global_step or early_stopping.early_stop:
                train_iterator.close()
                break
            writer.add_scalar("Loss/train", tr_loss / global_step, _)
            writer.add_scalar("KL Loss/train", tr_kl_loss / global_step, _)
    ...
    def evaluate(self, mode):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if self.args.model_type != "distilbert":
                    inputs["token_type_ids"] = batch[2]
                outputs = self.model(**inputs)
                tmp_eval_loss, tmp_eval_kl_loss, (intent_logits) = outputs[:3]
                # tmp_eval_loss, (intent_logits, slot_logits) = outputs[:2]
                eval_loss += tmp_eval_loss.mean().item()
                eval_kl_loss += tmp_eval_kl_loss.mean().item()
            nb_eval_steps += 1
        ...
        eval_kl_loss = eval_kl_loss / nb_eval_steps
        results = {"loss": eval_loss}
        results['KL loss'] =  eval_kl_loss
        ...
        total_result = compute_metrics_intent(
